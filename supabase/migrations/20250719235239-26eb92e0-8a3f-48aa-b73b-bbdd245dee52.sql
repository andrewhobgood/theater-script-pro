...
CREATE POLICY "Users can view their own licenses" 
    ...
    USING (licensee_id IN (
        ...
        SELECT scripts.id FROM public.scripts 
        JOIN public.profiles ON scripts.playwright_id = profiles.id 
...
CREATE POLICY "Anyone can view published reviews" 
    ...
    USING (script_id IN (
        SELECT scripts.id FROM public.scripts WHERE scripts.status = 'published'
    ));
